...
export const defaultDefs = {
    ...
    toLowerCase: {
        sanitizer: () => (x) => x.toLowerCase(),
    },
    toUpperCase: {
        sanitizer: () => (x) => x.toUpperCase(),
    },
    minLength: {
        ...
        validator: (props, context) => {
            ...
            if (async) {
                ...
                return async (v, opts) => {
                    const errors = opts.errors;
                    const path = opts.path || [];
                    const promises = stringKeys.map(([key, sani]) => sani.run(v[key], Object.assign(Object.assign({}, opts), { path: [...path, key] })));
                        ...
                        if (stringKeyMap[key] !== 1) {
                            for (let j = 0, jl = patternKeys.length; j < jl; j += 1) {
                                ...
                                if (regex.test(key)) {
                                    ...
                                    promises.push(sani.run(v[key], Object.assign(Object.assign({}, opts), { path: [...path, key] })));
                                    break;
                                }
                            }
                        }
            ...
            return (v, opts) => {
                ...
                for (let i = 0, l = stringKeys.length; i < l; i += 1) {
                    ...
                    const res = sani.runSync(v[key], Object.assign(Object.assign({}, opts), { path: [...path, key] }));
                    value[key] = res.value;
                    if (errors.length >= maxErrors) {
                        return { fatal: false, errors, value };
                    }
